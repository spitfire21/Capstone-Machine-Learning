...
Utilizes a backend / passhashing provided by bottle-cork for mongodb
https://github.com/FedericoCeratto/bottle-cork
Modified code to allow integration with mongodb backend, twilio, and account creation / authentication.
"""
...
auth_fields = collections.namedtuple('AuthField', 'user_id, password, email, phone, code')
...
class JWTProvider(object):
	...
	def __init__(self, fields, backend, secret, ssid, twilio, id_field='email_addr', algorithm='HS256', ttl=None):
	...
	def validate_token(self, token=''):
		...
		try:
			...
			print(base64.b64decode(user_uid))
	...
	def register(self, request):
		...
		if request.content_type.startswith('application/json'):  # pragma: no cover
			try:
				username = request.json.get(self.user_field.email)
				password = request.json.get(self.user_field.password)
				phone = request.json.get(self.user_field.phone)
			except (AttributeError, json.decoder.JSONDecodeError):
				raise JWTAuthError("Unable to create User")
		...
		try:
				...
				user = self.backend.register(username, password, username, phone, self.ssid, self.twilio)
		except:
				raise JWTAuthError("Unable to create User")

		if user:
			return {"Message":"User Created / Please login"}

		raise JWTAuthError("Unable to create User")



	def authenticate_code(self, request):
		if request.content_type.startswith('application/json'):  # pragma: no cover
			try:
				code = request.json.get(self.user_field.code)

			except (AttributeError, json.decoder.JSONDecodeError):
				raise JWTAuthError("Unable to create User")

		else:
			username = request.forms.get(self.user_field.code)

		try:
			user = self.backend.validate_registration(code)

		except:
			raise JWTAuthError("Code was not valid")

		if user:
			return {"Message":"Validation succesful"}
	...
	def authorize(self, request):
		...
		return self.validate_token(user_token['token']) or False
...
class JWTProviderPlugin(object):
	...
	def __init__(self, keyword, auth_endpoint, register_endpoint, validate_endpoint, login_enable=True, scope='plugin', **kwargs):
	...
	def setup(self, app):  # pragma: no cover
		...
		if self.login_enable:
			...
			@app.post(self.register_endpoint)
			def register_handler():
				try:

					response = self.provider.register(bottle.request)
					return response
			...
			@app.post(self.validate_endpoint)
			def code_handler():
				try:

					response = self.provider.authenticate_code(bottle.request)
